...
export class TokenGenerationService {
  ...
  async createAccessToken(payload: CreateAccessTokenProps): Promise<string> {
    const { accessSecret, accessExpireTime, issuer } = this.configService.get<JwtConfig>(ConfigKeys.Jwt);
  ...
  async createRefreshToken(payload: CreateRefreshTokenProps): Promise<string> {
    const { refreshSecret, refreshExpireTime, issuer } = this.configService.get<JwtConfig>(ConfigKeys.Jwt);
